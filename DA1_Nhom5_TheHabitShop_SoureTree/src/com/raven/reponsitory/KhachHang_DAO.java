...
public class KhachHang_DAO {
    ...
    public List<KhachHang> selectAllPhanTrang(long trang) {
        //throw new UnsupportedOperationException("Not supported yet."); // Generated from nbfs://nbhost/SystemFileSystem/Templates/Classes/Code/GeneratedMethodBody
        List<KhachHang> list = new ArrayList<>();
        sql = "select top 5  ma , ten , gioitinh,ngaysinh,sdt,email,diachi from KhachHang where ma not in (select top " + (trang * 5 - 5) + " ma from KhachHang ) order by KhachHang.ngaytao desc ";
        try {
            con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
